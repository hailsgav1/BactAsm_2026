...
def get_arg(argv=None):
    ...
    parser.add_argument('-l', '--list', metavar='', help="input list (provide each sample's SampleID and sraID in a row, separated by TAB)", default="", required=False)
...
def _get_samples(arguments):
    sample_dict={}
    if arguments.sra == "" and arguments.sampleID == "" and arguments.list != "":
        ...
        sys.exit("Error: Please provide SRA accession ID")
    else:
...
def main():
    BactAsm_path = os.path.abspath(os.path.dirname(__file__))
    arguments = get_arg()
    config = generate_config(arguments)
    thread = arguments.thread
    config.update(config)
    os.makedirs(os.path.join(BactAsm_path, "config"), exist_ok=True)
    config_file = os.path.join(BactAsm_path, "config", "config.yaml")
    with open(config_file, "w") as configfile:
        yaml.dump(config, configfile)
    os.chdir(BactAsm_path)
